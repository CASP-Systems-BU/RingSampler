# This code is from
# https://github.com/gpzlx1/gsampler-artifact-evaluation/blob/main/figure7/gSampler/graphsage_gSampler.py
...
import tqdm
import csv
from load_graph_utils import load_ogbn_products, load_100Mpapers, load_friendster, load_synthetic, load_yahoo
...
def benchmark_w_o_relabel(args, matrix, nid):
    ...
    with open("./result.csv", "a") as f:
        ...
        print(f"result writen to ./result.csv")
...
def load(dataset, args):
    ...
    csc_indptr, csc_indices, edge_ids = g.adj_tensors(fmt='csc')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--no-use-uva",
                        dest="use_uva", 
                        action="store_false",
                        help="Whether not to use UVA to sample graph and load feature")
    parser.set_defaults(use_uva=False)

    parser.add_argument("--dataset",
                        default='papers100m',
                        choices=[
                            'products', 'papers100m', 'friendster', 'synthetic', 'yahoo' 
                        ],
